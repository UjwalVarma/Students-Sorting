...
{"id":100,"first_name":"Zoe","last_name":"Moorhead","email":"zmoorhead2r@sfgate.com","gender":"Female","img_src":"https://robohash.org/fugiatcorporisdeleniti.png?size=50x50&set=set1","class":8,"marks":43,"passing":true,"city":"Makin Village"}
    ...
    const passingBtn = document.getElementById('passing');
    const classNoBtn = document.getElementById('classNo');
    ...
    ascendingBtn.addEventListener('click', () => {
      students.sort(sortArray('first_name', 'asc'));
      renderTable(students);
    });
  
    descendingBtn.addEventListener('click', () => {
      students.sort(sortArray('first_name', 'desc'));
      renderTable(students);
    });
  
    marksBtn.addEventListener('click', () => {
      students.sort(sortArray('marks', 'asc'));
      renderTable(students);
    });
    ...
    classNoBtn.addEventListener('click', () => {
      students.sort(sortArray('class', 'asc'));
      renderTable(students);
    });
